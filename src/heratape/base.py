...
def get_db_url(testing=False):
    ...
    if testing:
        db_url = config_data.get("test_db_url")
    else:
        db_url = config_data.get("default_db_url")
    if "postgresql" in db_url and "postgresql+psycopg" not in db_url:
        db_url = db_url.replace("postgresql", "postgresql+psycopg")
    if db_url is None:  # pragma: no cover
        raise RuntimeError(
            "cannot get heratape database file: no default_db_url "
...
class DB(metaclass=ABCMeta):  # noqa B024
    ...
    def __init__(self, testing=False):  # noqa
        db_url = get_db_url(testing=testing)
    ...
    def __init__(self, testing=False):
        ...
        with self.sessionmaker() as session:
            ...
            if not db_valid:  # pragma: no cover
                db_file = get_db_url(testing=testing)
                raise RuntimeError(
