...
from heratape.tape_system import (
    tape_in_drive,
    query_tape_jukebox,
    find_empty_slots,
    unload_tape,
...
with HTSessionWrapper(testing=TESTING) as ht_sess:
    unfinished_tapes = (
        ht_sess.query(func.distinct(Files.tape_id))
        .where(Files.write_date == None)
        .all()
    )
    logger.info(f"found {len(unfinished_tapes)} tapes with unfinished business")
    for utape in unfinished_tapes:
        unfinished_jds = (
            ht_sess.query(func.distinct(Files.jd))
            .where(Files.write_date == None)
            .where(Files.tape_id == utape[0])
            .all()
        )
        unfinished_jds = [
            ujd[0] for ujd in unfinished_jds
        ]  # why do I get a list of length 1 tuples??
...
if IN == "A":
    ...
    sys.exit()
if IN != "R":
    logger.info("Enter R to continue, or A to abort")
    sys.exit()
...
with HTSessionWrapper(testing=TESTING) as ht_sess:
    unfinished_files = (
        ht_sess.query(Files.filepath)
        .where(Files.write_date == None)
        .where(Files.tape_id == mytape)
        .all()
    )
    unfinished_files = [
        uff[0] for uff in unfinished_files
    ]  # why a len 1 tuple. again why
...
for i in np.arange(len(unfinished_files)):
    F.write(f"{unfinished_files[i]}\n")
...
if not TESTING:
    ...
    with subprocess.Popen(
        f"time tar -cf /dev/nst{mydrive} -T {filelistfile} ",
        shell=True,
